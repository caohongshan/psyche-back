...
class T {
	...
	async _createToken({
		...
		const o = Date.now(),
			...
			u = await this.getUserRecord(),
			d = (u.token || []).filter(e => {
				try {
					const t = this._checkToken(e);
					if (u.valid_token_date && u.valid_token_date > 1e3 * t.iat) return !1
				} catch (e) {
					if (e.errCode === n.TOKEN_EXPIRED) return !1
				}
				return !0
			});
		return d.push(a), await this.updateUserRecord({
...
for (const e in E) A.prototype[e] = E[e];
